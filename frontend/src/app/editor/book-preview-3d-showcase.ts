...
@Component({
  selector: 'app-book-preview-3d-showcase',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './book-preview-3d-showcase.html',
  styleUrl: './book-preview-3d-showcase.css',
})
export class BookPreview3dShowcaseComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  private async rebuildModelIfNeeded(forceReload: boolean): Promise<void> {
    ...
    try {
      ...
      console.error('3D showcase model loading failed:', error);
      if (requestId === this.loadRequestId) {
        this.disposeCurrentModel();
  ...
  private resolveSemanticMeshTextureUrls(
    ...
    if (family === 'FLAT_2P' || family === 'SADDLESTITCH') {
      return {
        frontUrl: (map?.flatPage1 ?? map?.outsideFront ?? rawFallback) ?? undefined,
  ...
  private fitCameraToModel(): void {
    ...
    const distance = (maxSize * 0.52) / Math.tan(THREE.MathUtils.degToRad(this.camera.fov * 0.5));
  ...
  private createFallbackModel(family: BookPreviewFamily): THREE.Group {
    ...
    if (family === 'WRAP_1P' || family === 'WRAP_2P') {
      ...
      spine.name = 'Spine';
      group.add(spine);
      this.tagOwnedMaterial(spine.material);
    }
