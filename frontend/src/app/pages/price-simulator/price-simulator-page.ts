...
import { PricingService, QuoteResponse, QuoteRequest } from '../../core/services/pricing.service';
...
export class PriceSimulatorPageComponent implements OnInit {
  ...
  readonly quoteResponse = signal<QuoteResponse | null>(null);
  readonly isLoading = signal(false);
  readonly manualCalculationDone = signal(false);
  ...
  readonly estimatedUnitPrice = computed(() => {
    return this.quoteResponse()?.selectedPrice ?? 0;
  });

  readonly productionSubtotal = computed(() => {
    return (this.quoteResponse()?.selectedPrice ?? 0) * this.quantity();
  });

  readonly setupFee = computed(() => {
    return this.quoteResponse()?.pricingDetails?.setupFee ?? 0;
  });

  readonly addOnTotal = computed(() => {
    return this.quoteResponse()?.pricingDetails?.addOnTotal ?? 0;
  });

  readonly rushFee = computed(() => {
    return this.quoteResponse()?.pricingDetails?.rushFee ?? 0;
  });

  readonly estimatedTotal = computed(() => {
    return this.productionSubtotal() + this.setupFee() + this.addOnTotal() + this.rushFee();
  });
  ...
  calculatePrice(): void {
    this.manualCalculationDone.set(true);
    this.isLoading.set(true);

    const hasCoil = this.shouldShowCoilType() ? 1 : 0;
    const hasInsert = this.shouldShowInsertPaperType() ? 1 : 0;
    const hasTab = this.shouldShowTabColor() ? 1 : 0;
    const hasBackcover = this.hasBackcoverFromBinding() ? 1 : 0;

    const quoteRequest: QuoteRequest = {
      siren: this.auth.getUserSiren?.() ?? null,
      ...
      product: {
        ...
        securityLabel: this.securityLabel() ? 1 : 0,
        hasCoil: hasCoil,
        hasInsert: hasInsert,
        hasTab: hasTab,
        hasBackcover: hasBackcover,
        perf: this.perf() ? 1 : 0,
        doubleSidedCover: this.doubleSidedCover() ? 1 : 0,
        shrinkwrap: this.shrinkwrap() ? 1 : 0,
        threeHoleDrill: this.threeHoleDrill() ? 1 : 0,
        ...
        insertPaperType: this.insertPaperType(),
        caseFinishType: this.caseFinishType(),
        spineType: this.spineType(),
    ...
    this.pricingService.getQuote(quoteRequest).subscribe({
      next: (response) => {
        this.quoteResponse.set(response);
        this.isLoading.set(false);

        if (!response.available && response.message) {
          console.warn('Quote not available:', response.message);
        }
      },
      error: (error) => {
